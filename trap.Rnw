...
<<label=gev,echo=False,fig=True,eval=False>>=
...
<<label=affinity_upregulation,echo=False,fig=True,eval=False>>=
...
<<label=tf_scan,echo=False,fig=False,cache=True,eval=False>>=
for (tf.seq in dir('~/data/hdf/tf_seq/')) {
    seq.file <- paste('~/data/hdf/tf_seq/',tf.seq,sep='')
    res.file <- paste('~/data/hdf/tf_seq/',sub('fa','trap',tf.seq),sep='')
    system(paste('./TRAP /home/sohse/TFP_2012.1_data/dat/matrix.dat',seq.file,
        '>',res.file))
}
@
...
<<label=tf_network,echo=False,fig=False,eval=False>>=
tf.symbol <- 'IRF_|IRF7|CEBPD|EMX2|MEF2D|NFE2|NFK|RELB|RUNX1|STAT[134]'
idx <- grep(tf.symbol, gev.param$X.matrix)
L <- 1000
all <- NULL
for (res.file in dir('~/data/hdf/tf_seq/','*.trap')) {
    res <- read.delim(paste('~/data/hdf/tf_seq/',res.file,sep=''), header=F)
    ...
    df$tf <- sub('.trap', '', res.file)
    all <- rbind(all, df)
}

pdf('~/data/hdf/hdf-tf_scan.pdf',8,11)
unique.tf <- levels(factor(all$tf))
for (i in 1:ceiling(length(unique.tf)/12)) {
    slice <- seq((i-1)*12+1,i*12)
    slice <- slice[!(slice > length(unique.tf))]
    current <- subset(all, tf %in% unique.tf[slice])
    print(ggplot(current,aes(-log10(pval),V4)) + 
        geom_point() +
        facet_wrap(~tf,ncol=3) +
        geom_text(aes(label=V1),size=2,hjust=-0.1,alpha=0.3) +
        #geom_text(data=pval.cebpd[grep('ATF|NFK',pval.cebpd$V1),],aes(label=V1),
        #    size=3,hjust=-0.2,colour='red') +
        xlim(0,6) +
        labs(x=expression(paste(-log[10]~italic(p),'-value')), 
...
<<label=profile,echo=False,fig=True,eval=True>>=
res <- read.delim('~/github/TRAP/human_cebpd.trap', header=F)
res <- res[grep('ATF3|NFKAPPAB',res$V1),]
df <- NULL
for (i in 1:nrow(res)) {
...
print(ggplot(df,aes(pos-1001,affinity,colour=matrix)) + geom_line() +
    ...
    labs(x='Bases relative to TSS',y='Affinity',
        title='Affinity profile of CEBPD promoter region\n') +
    theme(legend.position=c(0.2,0.8),legend.key=element_blank(),
